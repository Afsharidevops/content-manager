...
Do not publish a redundant `v0.4.2` Docker tag.
...
3. Pushes `afsharidevops/content-bot:0.4.2` and `:latest` to Docker Hub.
4. Verifies the published multi-platform manifest.

The workflow can also be started manually from the GitHub Actions tab
(`workflow_dispatch`); manual runs publish the same `0.4.2` tag from `main`.
