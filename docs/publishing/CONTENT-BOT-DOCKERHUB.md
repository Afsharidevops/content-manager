...
Do not publish a redundant `v0.4.3` Docker tag.
...
3. Pushes `afsharidevops/content-bot:0.4.3` and `:latest` to Docker Hub.
4. Verifies the published multi-platform manifest.

The workflow can also be started manually from the GitHub Actions tab
(`workflow_dispatch`); manual runs publish the same `0.4.3` tag from `main`.
